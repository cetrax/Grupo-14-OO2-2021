	...
	public LoteModel Insert(LoteModel loteModel) {
      Localito loc = localRepository.findByIdLocal(loteModel.getLocal().getIdLocal());
		...
		if(stock==null ){
			...
			stock.setCantidad(stock.getCantidad()+lote.getCantidadActual());//**********************
		}
		stock.setLocal(loc);
		lote.setStock(stock);
...
//		loteModel.setCantidadActual(loteModel.getCantidadActual());
...
//	}
	...
	public LoteModel Update(LoteModel loteModel) {
		...
		if(loteOrig.getCantidadActual()<=loteModel.getCantidadActual()){
			int cantidad = loteOrig.getCantidadActual() - loteModel.getCantidadActual();
			Stock stock = stockRepository.findByIdStock(loteModel.getStock().getIdStock());
			stock.setCantidad(loteOrig.getStock().getCantidad() - cantidad );;
			loteOrig.setStock(stock);		
		}
